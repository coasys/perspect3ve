...
export class ExpressionWidget {
  ...
  async addGraphAndText() {
    ...
    if (this.#base == 'ad4m://self') {
      createPerspectiveBackground(this.#graphic).then((sprite) => {
        this.#backgroundContainer.addChild(sprite);
      });
    }
    if (this.#base.startsWith('flux://')) {
      createBlobBackground(this.#graphic, this.#graphicMask, 0x44bb33).then((sprite) => {
        this.#backgroundContainer.addChild(sprite);
      });
    }
    if (this.#base.startsWith('flux_entry://')) {
      createBlobBackground(this.#graphic, this.#graphicMask, 0xffffff).then((sprite) => {
        this.#backgroundContainer.addChild(sprite);
      });
    }
    this.updateDisplayText();
  ...
  async updateDisplayText() {
    ...
    if (this.#graphicMask) this.#text.mask = this.#graphicMask;
  ...
  async updateBackground() {
    ...
    if (this.#subjectProxy && this.#subjectProxy.image) {
      ...
      if (image) {
        if (image.data_base64) background_file_expression = image;
        else background_url = image;
      }
    }

    if (!background_file_expression && !background_url) {
      for (const predicate of BACKGROUND_PREDICATES) {
        const background_links = await this.#perspective.get(
          new LinkQuery({
            source: this.#base,
            predicate: predicate
          })
    ...
    if (background_file_expression) {
      ...
      if (background_file_expression.data_base64) {
        sprite = await createBackgroundFromBase64(
          this.#graphic,
          this.#graphicMask,
          background_file_expression.data_base64
        );
      } else {
        sprite = await createBackgroundFromFileExpression(
          this.#graphic,
          this.#graphicMask,
          background_file_expression
        );
      }
  ...
  async #updateSubjectClass() {
    try {
      const classResults = await this.#perspective.infer(
        `subject_class(ClassName, C), instance(C, "${this.base}").`
      );
      if (classResults && classResults.length > 0) {
        ...
        await tryForEachMatch(
          ...
          (shapeResults) => {
            if (
              shapeResults.Shape &&
              typeof shapeResults.Shape == 'string' &&
              shapeResults.Shape != 'none'
            )
        ...
        await tryForEachMatch(
          ...
          (colorResults) => {
            if (
              colorResults.Color &&
              typeof colorResults.Color == 'string' &&
              colorResults.Color != 'none'
            ) {
        ...
        if (this.#subjectShape || this.#subjectColor) await this.#updateExpressionGraphic();
      } else {
  ...
  async getSubjectPropertyExpressions(): Promise<string[]> {
    const result = await this.#perspective.infer(
      `subject_class("${this.#isInstanceOfSubjectClass}", C), property_getter(C, "${
        this.base
      }", _, Property).`
    );
  ...
  async updateChildrenCoords() {
    const hiddenExpressions = new Set<string>();
    for (const predicate of HIDDEN_PREDICATES) {
      const hiddenLinks = await this.#perspective.get(
        new LinkQuery({
          source: this.#base,
          predicate: predicate
        })
    ...
    const result: LinkExpression[] = await this.#perspective.get(
      new LinkQuery({ source: this.#base })
    );
    for (const link of result) {
      ...
      const childrenCoords = this.#childrenCoords.get(child);
      if (childWidget && childrenCoords) childWidget.#relativePosition = childrenCoords;
    }
  }
  ...
  addChild(child: string, point: { x: number; y: number }): ExpressionWidget {
    ...
    const childLayer = new PIXI.Container();
    childLayer.scale.set(LEVEL_SCALE);
    childLayer.position.set(point.x, point.y);

    const childWidget = new ExpressionWidget(
      child,
      this.#perspective,
      childLayer,
      this.#canvasSize
  ...
  injectChildWidget(child: ExpressionWidget) {
    ...
    const baseCoords = this.#childrenCoords.get(child.base);
    if (baseCoords) {
      child.#relativePosition = baseCoords;
    }
        ...
        if (that.#isDragging) {
          if (that.#dragOver) {
            ...
            const pointCopy = JSON.parse(
              JSON.stringify({
                x: childWidget.container.position.x,
                y: childWidget.container.position.y
              })
            );
  ...
  async #findCoordsLink(expr: string): Promise<LinkExpression | undefined> {
    const results = await this.#perspective.get(
      new LinkQuery({ source: this.#base, target: expr })
    );
  ...
  #drawExpressionSticky(graphic: PIXI.Graphics, color = 0xffffcc) {
    graphic.clear();
    graphic.beginFill(color, this.#selected ? 0.5 : 0.2);
    if (this.#selected) graphic.lineStyle(OUTLINE_WIDTH_SELECTED, OUTLINE_COLOR_SELCTED);
    else graphic.lineStyle(OUTLINE_WIDTH, OUTLINE_COLOR);
  ...
  #drawExpressionCircle(graphic: PIXI.Graphics, color = 0x0000ff) {
    graphic.clear();
